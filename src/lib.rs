...
// This is needed in e-matching if you traverse through certain kinds of redundant edges
...
impl EGraph {
    ...
    // -----------------------
    // Application
    // Pattern instantiation and object variable substitution
    // --------------------------
    ...
    fn instantiate_metavars_binding(
        &mut self,
        binding: &Id,
        top_ctx: usize,
        current_ctx: usize,
    ...
    /// Instantiate a pattern's metavariables from a match substitution.
    /// This is distinct from [`Self::substitute`], which replaces an object
    /// variable and performs capture-avoiding traversal beneath binders.
    pub fn try_instantiate_metavars(
        &mut self,
        pattern: &Pattern,
        ctx: usize,
        subst: &Subst,
        ...
        match pattern {
            Pattern::MetaVar(name, arguments) => {
                ...
                let arguments: Option<SmallVec<[Id; 2]>> = arguments
                    .iter()
                    .map(|argument| {
                        self.try_instantiate_metavars_rec(argument, ctx, top_ctx, subst)
                    })
                    .collect();
                self.instantiate_metavars_binding(&binding, top_ctx, ctx, &arguments?)
            }
            ...
            Pattern::App(function, argument) => {
                let function = self.try_instantiate_metavars_rec(function, ctx, top_ctx, subst)?;
                let argument = self.try_instantiate_metavars_rec(argument, ctx, top_ctx, subst)?;
                Some(self.app(function, argument))
            }
            Pattern::Binder(op, body) => {
                let body = self.try_instantiate_metavars_rec(body, ctx + 1, top_ctx, subst)?;
                Some(self.binder_symbol(*op, body))
            }
            Pattern::Subst(body, replacement) => {
                let body = self.try_instantiate_metavars_rec(body, ctx + 1, top_ctx, subst)?;
                let replacement =
                    self.try_instantiate_metavars_rec(replacement, ctx, top_ctx, subst)?;
    ...
    /// Return the same canonical e-class view as [`Self::dump`], represented
    /// as ordinary serializable data instead of formatted text.
    pub fn json_dump(&mut self) -> EGraphDump {
        fn dump_id(id: Id) -> DumpId {
            DumpId {
                raw: id.raw(),
                thinning: (0..id.ctx()).map(|index| id.lift().get(index)).collect(),
            }
        }
        fn dump_node(node: Node) -> ENodeKind {
            match node {
                Node::Var => ENodeKind::Var,
                Node::Atom(name) => ENodeKind::Atom {
                    name: name.as_str().to_owned(),
                },
                Node::App(function, argument) => ENodeKind::App {
                    function: dump_id(function),
                    argument: dump_id(argument),
                },
                Node::Binder(operator, body) => ENodeKind::Binder {
        ...
        self.rebuild();
        let roots: Vec<_> = (0..self.parent.len() as RawId)
            .filter(|&raw| self.parent[raw as usize].raw() == raw)
            .map(|raw| {
                (
                    raw,
            ...
            .collect();
        let classes: Vec<_> = roots
            .into_iter()
            .map(|(raw, context, nodes)| EClassDump {
                id: raw,
                context,
                representative: self.extract(&Id::new(Lift::identity(context), raw)),
                nodes: nodes
                    .into_iter()
                    .map(|(node, lift)| ENodeDump {
                        placement: dump_id(Id::new(lift, raw)),
                        node: dump_node(node),
                    })
                    .collect(),
            })
...
// ---------------------------------------------------
// These are things I want deemphasized.
// De Bruijn Indices and levels are necessary due to translation from surface syntax really
...
/// A stable, serialization-friendly snapshot produced by `print-egraph` in
/// JSON mode. It intentionally omits union-find and memo-table internals.
#[derive(Clone, Debug, Serialize)]
pub struct EGraphDump {
    pub class_count: usize,
    pub node_count: usize,
    pub classes: Vec<EClassDump>,
}

#[derive(Clone, Debug, Serialize)]
pub struct EClassDump {
    pub id: RawId,
    pub context: usize,
    pub representative: Option<TermCtx>,
    pub nodes: Vec<ENodeDump>,
}

#[derive(Clone, Debug, Serialize)]
pub struct DumpId {
    pub raw: RawId,
    pub thinning: Vec<bool>,
}

#[derive(Clone, Debug, Serialize)]
pub struct ENodeDump {
    pub placement: DumpId,
    #[serde(flatten)]
    pub node: ENodeKind,
}
...
pub enum ENodeKind {
    Var,
    Atom { name: String },
    App { function: DumpId, argument: DumpId },
    Binder { operator: String, body: DumpId },
}
